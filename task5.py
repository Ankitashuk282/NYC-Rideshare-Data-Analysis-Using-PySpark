...
from pyspark.sql.functions import month, dayofmonth, avg
...
if __name__ == "__main__":
    ...
    january_df = rideshare_pickup_dropoff_df.filter(month('date') == 1)
    
    # Group by the day of the month and calculate the average waiting time for each day
    average_waiting_time_by_day = january_df.groupBy(dayofmonth('date').alias('day')).agg(avg('request_to_pickup').alias('avg_waiting_time'))
    
    # Sort the results by day for better readability
    average_waiting_time_by_day = average_waiting_time_by_day.orderBy('day')
    
    # Show the results
    average_waiting_time_by_day.show(50, truncate=False)

    output_path = "s3a://" + s3_bucket + "/task5/waiting_times_january"
    average_waiting_time_by_day.write.option("header", "true").csv(output_path)
